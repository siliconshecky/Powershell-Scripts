...
#Start search
# enter the OU path for where to search. To not search sublevels add -SearchScope OneLevel before -searchbase
$computers = Get-ADComputer -Filter {Enabled -eq 'true' -and SamAccountName -like $Prefix -and operatingsystem -like 'Windows 7 Professional'} -Searchbase 'OU=,DC=,DC=,DC=com'
	...
	$Reply = $null
  	...
  	if($Reply -eq 'True'){
		if($Computer -eq $env:COMPUTERNAME){
			...
			$proc = gwmi win32_process -ErrorAction SilentlyContinue -Authentication PacketPrivacy -Impersonation Impersonate -Credential $Credential -computer $Computer -Filter "Name = 'explorer.exe'"
		}			
